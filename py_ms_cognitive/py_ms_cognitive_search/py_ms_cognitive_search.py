...
class PyMsCognitiveSearch(object):
    ...
    def get_json_results(self, response):
        '''Parses the request result and returns the JSON object.

        Handles all errors.
        '''
        try:
            # return the proper JSON object, or error code if request didn't go
            # through.
            self.most_recent_json = response.json()
            json_results = response.json()
            # 401 is invalid key, 403 is out of monthly quota.
            if response.status_code in [401, 403]:
                raise PyMsCognitiveException(
                    "CODE {code}: {message}".format(
                ...
                try:
                    ...
                    timeout = int(
                        re.search('in (.+?) seconds', message).group(1)) + 1
                    print("CODE 429, sleeping for {timeout} seconds").format(
                        timeout=str(timeout))
                    time.sleep(timeout)
                except (AttributeError, ValueError):
                    if not self.safe:
                        raise PyMsCognitiveException(
                            "CODE 429. Failed to auto-sleep: {message}".format(
                                code=response.status_code,
                                message=json_results["message"]
                            )
                        )
                        ...
                        print(
                            "CODE 429. Failed to auto-sleep: {message}. "
                            "Trying again in 5 seconds.".format(
                                code=response.status_code,
                                message=json_results["message"]
                            )
                        )
            ...
            if not self.safe:
                raise PyMsCognitiveException(
                    "Request returned with code %s, error msg: %s".format(
                        response.status_code, response.text
                    )
                )
            else:
                print(
                    "[ERROR] Request returned with code %s, error msg: %s. \n"
    ...
    def search(self, limit=50, format='json'):
        '''Returns the result list, and also the uri for next page.

        Format (returned_list, next_uri)
        '''
...
class QueryChecker():
    ...
    def check_web_params(query_dict, header_dict):
        responseFilters = (
            'Computation', 'Images', 'News', 'RelatedSearches',
            'SpellSuggestions', 'TimeZone', 'Videos', 'Webpages')

        if 'cc' in query_dict.keys():
            if query_dict['cc'] and not header_dict['Accept-Language']:
                raise AssertionError(
                    'Attempt to use country-code without specifying language.')
            if query_dict['mkt']:
                raise ReferenceError(
                    'cc and mkt cannot be specified simultaneously')
        if 'count' in query_dict.keys():
            if int(query_dict['count']) >= 51 or int(query_dict['count']) < 0:
                raise ValueError(
                    'Count specified out of range. 50 max objects returned.')
        if 'freshness' in query_dict.keys():
            if query_dict['freshness'] not in ('Day', 'Week', 'Month'):
                raise ValueError(
                    'Freshness must be == Day, Week, or Month. Assume Case-Sensitive.')
        ...
        if 'safeSearch' in query_dict.keys():
            if query_dict['safeSearch'] not in ('Off', 'Moderate', 'Strict'):
                raise ValueError(
                    'safeSearch setting must be Off, Moderate, or Strict. Assume Case-Sensitive.')
            if 'X-Search-ClientIP' in query_dict.keys():
                raw_input(
                    'You have specified both an X-Search-ClientIP header and safesearch setting\nplease note: header takes precedence')
        if 'setLang' in query_dict.keys():
            if header_dict['Accept-Language']:
                raise AssertionError(
                    'Attempt to use both language header and query param.')
        ...
        if 'textFormat' in query_dict.keys():
            if query_dict['textFormat'] not in ('Raw', 'HTML'):
                raise ValueError(
                    'textFormat must be == Raw or HTML. Assume Case-Sensitive.')
        return True
